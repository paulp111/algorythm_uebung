...
const testStrings = ["radar", "hello", "level"];
testStrings.forEach(testString => {
    console.log(`Ist "${testString}" ein Palindrom? ${isPalindrome(testString)}`);
}); //true, false, true
...
function someRecursive(array: number[], callback: (element: number) => boolean): boolean {
    if (array.length === 0) {
        return false;
    }
    if (callback(array[0])) {
...
const result = someRecursive(array, isEven);
...
const capitalizedArray = capitalizeFirst(testArray);
...
const sumOfEvens = nestedEvenSum(testObj);
